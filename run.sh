...
# Define Kaldi
KALDI_ROOT=~/Desktop/Workspace/kaldi
PATH=$PWD/utils/:$KALDI_ROOT/tools/openfst/bin:$KALDI_ROOT/tools/sph2pipe_v2.5:$PWD:$PATH
...
sid=$KALDI_ROOT/egs/sre08/v1/sid
...
if [ $stage -le 2 ]; then
  ...
  $sid/train_full_ubm.sh --cmd "$train_cmd --mem 24G" \
...
if [ $stage -le 3 ]; then
  ...
  $utils/subset_data_dir.sh \
    --utt-list <(sort -n -k 2 data/train/utt2num_frames | tail -n 50000) \
    data/train data/train_50k
  # Train the i-vector extractor.
  $sid/train_ivector_extractor.sh --cmd "$train_cmd --mem 16G" \
    --ivector-dim 50 --num-iters 5 \
    exp/full_ubm/final.ubm data/train_50k \
    exp/extractor
  echo "==================stage 3 finished========================="
fi

if [ $stage -le 4 ]; then
  $sid/extract_ivectors.sh --cmd "$train_cmd --mem 16G" --nj 80 \
    exp/extractor data/train \
    exp/ivectors_train

  $sid/extract_ivectors.sh --cmd "$train_cmd --mem 16G" --nj 40 \
...
if [ $stage -le 7 ]; then
  ...
  # EER: 5.329%
  # (p-target=0.01): 0.4933
  # minDCF(p-target=0.001): 0.6168
fi

# TRIAL 1
# i-vector data: {10k}, dimension: {50}
# extractor: g-select: {4}
# lda dimension: {50}

# result:
# EER: 8.86%
# minDCF(p-target=0.01): 0.7145
# minDCF(p-target=0.001): 0.7937
