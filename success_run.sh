...
if [ $stage -le 1 ]; then
  ...
  for name in train voxceleb1_test; do
    steps/make_mfcc.sh --write-utt2num-frames true \
      --mfcc-config conf/mfcc.conf --nj 10 --cmd "$train_cmd" \
...
if [ $stage -le 3 ]; then
  ...
  sid/train_ivector_extractor.sh --cmd "$train_cmd --mem 8G" \
    --ivector-dim 50 --num-iters 5 \
    exp/full_ubm/final.ubm data/train_10k \
    exp/extractor
  echo "==================stage 3 finished========================="
fi

if [ $stage -le 4 ]; then
  sid/extract_ivectors.sh --cmd "$train_cmd --mem 8G" --nj 10 --num-gselect 4\
    exp/extractor data/train \
    exp/ivectors_train

  sid/extract_ivectors.sh --cmd "$train_cmd --mem 8G" --nj 10 \
